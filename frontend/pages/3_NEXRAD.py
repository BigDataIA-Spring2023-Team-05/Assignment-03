...
def nexrad_ui():
    ...
    stcheck = requests.get("http://localhost:8000/nexrad/files", params = payload, headers=headers)
    output =  stcheck.json()
    # print(output)
    ...
    if st.button('Generate Link', key ='nexrad_filed_search'):
        ...
        token = st.session_state["authentication_status"]
        headers = {'Authorization': f'Bearer {token}'}
        url = 'http://localhost:8000/nexrad/generate/aws-link'
 ...
 ## Button code :
    ...
    if st.button('Generate the link',key = 'nexrad_file_search'):
        if match:
            ...
            token = st.session_state["authentication_status"]
            headers = {'Authorization': f'Bearer {token}'}
            result_status = requests.post(f"http://localhost:8000/nexrad/generate/aws-link-by-filename/{file_input}",headers = headers).status_code
            if result_status == 201:
            
                result = requests.post(f"http://localhost:8000/nexrad/generate/aws-link-by-filename/{file_input}",headers = headers).json()
