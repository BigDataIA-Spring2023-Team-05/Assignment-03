...
def goes_ui():
    ...
    # url = 'http://localhost:8000/goes/files'
    ...
    output = requests.get("http://localhost:8000/goes/files", params = payload, headers=headers).json()
    # print(output)
    ...
    if st.button('Generate Link', key ='goes_filed_search'):
        ...
        token = st.session_state["authentication_status"]
        headers = {'Authorization': f'Bearer {token}'}
        url = 'http://localhost:8000/goes/generate/aws-link'
    ...
    if st.button('Generate the link',key = 'goes_file_search'):
        if match:
            ...
            token = st.session_state["authentication_status"]
            headers = {'Authorization': f'Bearer {token}'}
            result_status = requests.post(f"http://localhost:8000/goes/generate/aws-link-by-filename/{file_input}",headers = headers).status_code
            if result_status == 201:
            
                result = requests.post(f"http://localhost:8000/goes/generate/aws-link-by-filename/{file_input}",headers = headers).json()
