...
from schemas.index import User, LoginResponse
...
def find_user(username: str, password: str, db: Session):
    ...
    if not user:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"User with the username '{username}' not found")
